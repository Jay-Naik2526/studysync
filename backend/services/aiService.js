    ...
    for (const modelName of MODELS) {
      try {
        ...
        const model = genAI.getGenerativeModel({ 
          model: modelName,
          systemInstruction: "You are an elite academic content engine. For notes/revision: produce exhaustive, well-structured Markdown with LaTeX math, tables, and code blocks. For flashcards/quiz: return ONLY a raw JSON array — no markdown fences, no preamble, no trailing text. Your JSON must be valid and parseable. Never truncate output."
        });
